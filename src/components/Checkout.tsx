...
export default function Checkout({ handleClick, topNickname }: PaymentFormProps) {
    ...
    <section className='w-full from-black-to-transparent p-5 h-full justify-center box-border border-secondary-black border flex flex-col blur-md rounded-md'>
      <div className="flex flex-col mb-2 items-center">
        ...
        <p className='text-gray-500 w-4/5 text-sm text-center mb-5'>To take the first place from
          <span className='text-white'> {topNickname ? topNickname.name : 'No one'} </span>
          you need to pay
          <span className='text-main'> {topNickname ? topNickname.amount / 100 : '0,00'} US$</span>
        </p>
      </div>
      <button
